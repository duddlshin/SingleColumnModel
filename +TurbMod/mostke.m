...
switch string(settings.casename)
    case {"tnbl", "channelflow"}
        ...
        C_3 = (1./zeta).*(settings.phi_M./settings.phi_H).*(params.C_1*settings.phi_M ...
            - params.C_2*settings.phi_E + (params.C_2 - params.C_1)*settings.phi_E.^(-0.5).*fe);  C_3(isnan(C_3)) = 0;
        % RHS in prognostic equation for TKE
    ...
    case{"sbl"}
        ...
        % RHS in prognostic equation for TKE
        S_k = (S.ustar^3/(settings.kappa*settings.L))*(1 - settings.phi_H./settings.phi_M - (C_kD/4)*settings.phi_M.^(-7/2).*settings.phi_E.^(-3/2).*fst);
        ...
        C_3 = (settings.Pr_t./zeta).*(settings.phi_M./settings.phi_H).*(params.C_1*settings.phi_M - params.C_2*settings.phi_E ...
            + (params.C_2 - params.C_1)*settings.phi_E.^(-0.5).*fe);  C_3(isnan(C_3)) = 0; 
        % disp(C_3);
        T_e = (S.ustar^4./(settings.kappa*settings.zCell).^2).*C_kD.*(params.sig_k/params.sig_e).*sqrt(params.C_mu).*sqrt(settings.phi_M).*fe;
    otherwise
